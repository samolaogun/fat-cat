...
fatCat = (function() {
  fatCat.sSize = window.innerWidth;

  fatCat.units = ['rem', 'em', 'vw', 'vh', 'px', '%'];

  function fatCat(elements, props, pairs, opts) {
    ...
    if (!fatCat.units.includes(this.unit)) {
    ...
    this.value = b1 + (fatCat.sSize - a1) * (b2 - b1) / (a2 - a1);
    if (this.clamp) {
      if (this.value > b2) {
        this.value = b2;
